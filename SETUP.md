...
Already set up? Copy-paste these in order. Run each block in its own terminal tab.

**Terminal 1 — Backend** (run from `cloud-brain/`)
```bash
# Start Docker (Postgres + Redis) — safe to run even if already up
docker compose up -d
...
This starts three containers: `zuralog-postgres` (port 5432), `zuralog-redis` (port 6379), and `zuralog-celery-worker` (worker + Beat scheduler combined). For initial development you only need Postgres and Redis healthy.
...
You should see `zuralog-postgres` and `zuralog-redis` with status `healthy`. The Celery container will start once both datastores are healthy.
...
| `WITHINGS_WEBHOOK_SECRET` | Leave **empty** for local development. The production value is set in Railway on both services (Zuralog, Celery_Worker) — copy from Bitwarden → "Withings Webhook Secret" if you need to test webhook delivery locally. |
...
#### Required: Polar AccessLink Integration

Polar AccessLink OAuth 2.0 is fully implemented with HMAC-SHA256 webhook verification, a mandatory user registration step, 14 MCP tools, and Celery sync tasks. The API application is already registered at [admin.polaraccesslink.com](https://admin.polaraccesslink.com) under `developer@zuralog.com`. You do not need to register a new application.

Get the credentials from Bitwarden (search **"Polar API - Zuralog"**) and copy them into `.env`:

| Variable | Description |
|---|---|
| `POLAR_CLIENT_ID` | From Bitwarden → "Polar API - Zuralog" → Username |
| `POLAR_CLIENT_SECRET` | From Bitwarden → "Polar API - Zuralog" → Password — **never commit** |
| `POLAR_REDIRECT_URI` | Keep default: `zuralog://oauth/polar` |
| `POLAR_API_BASE_URL` | Keep default: `https://api.zuralog.com`. Override for staging: `https://staging-api.zuralog.com` |
| `POLAR_WEBHOOK_SIGNATURE_KEY` | Leave **empty** for local development. See note below. |

> **Webhook signature key:** Unlike Fitbit/Withings, the `POLAR_WEBHOOK_SIGNATURE_KEY` is **generated by Polar** when you register a webhook, not chosen by you. For local development, leave it empty — when the key is absent, signature verification is skipped entirely. In production, the key is created automatically when the `create_polar_webhook_task` Celery task runs on first user connection. Check the Railway worker logs for the line `IMPORTANT: Save this as POLAR_WEBHOOK_SIGNATURE_KEY env var: <key>`, then set it on both Railway services (Zuralog, Celery_Worker).

> **Unique Polar protocol requirements:** Unlike other integrations, Polar requires a mandatory user registration step (`POST /v3/users`) immediately after the OAuth token exchange, before any data can be accessed. The backend handles this transparently. Polar tokens last approximately one year (`expires_in: 31535999`) with no refresh token — when a token expires, the user must reconnect. The `monitor_polar_token_expiry_task` Celery task sends a push notification 30 days before expiry.
...
> **Production (Railway):** PostHog is disabled in production (cost optimization). No Railway action needed — `POSTHOG_API_KEY` is intentionally left empty on all Railway services.
...
| `SENTRY_TRACES_SAMPLE_RATE` | `1.0` (100% locally; set to `0.05` on Zuralog, `0.0` on Celery_Worker in Railway prod) |
| `SENTRY_PROFILES_SAMPLE_RATE` | `0.25` (25% locally; set to `0` on all Railway services) |
