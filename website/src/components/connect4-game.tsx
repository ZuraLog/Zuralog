 ...
 * - Peach-themed: peach (player) vs orange (AI)
 */
'use client';
...
const PLAYER_COLOR = '#FFAB76'; // peach
const AI_COLOR = '#FC4C02';     // orange
const BOARD_BG = '#FFF0E6';
const CELL_BG = '#FFE4CC';
...
export function Connect4Game() {
  ...
  const checkGameEnd = useCallback((newBoard: Board, piece: number) => {
    ...
    if (isBoardFull(newBoard)) { setGameState('draw'); return true; }
    return false;
  }, []);
  ...
  const doAIMove = useCallback((currentBoard: Board) => {
    ...
    setTimeout(() => {
      ...
      if (newBoard) {
        setBoard(newBoard);
        if (!checkGameEnd(newBoard, AI)) isPlayerTurn.current = true;
      }
      setAiThinking(false);
    }, 600);
  }, [checkGameEnd]);
  ...
  function handleColumnClick(col: number) {
    ...
    if (!checkGameEnd(newBoard, PLAYER)) doAIMove(newBoard);
  }
  ...
  useEffect(() => { isPlayerTurn.current = true; }, []);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between px-4 pb-2 pt-3">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-bold text-dark-charcoal">Connect 4</span>
          <span className="rounded-full border border-black/10 bg-black/5 px-1.5 py-0.5 text-[8px] font-semibold text-black/40">
            vs AI
          </span>
        </div>
      ...
      <div className="flex items-center justify-center gap-2 px-4 pb-2">
        {gameState === 'playing' ? (
          <>
            <div className="h-2 w-2 rounded-full" style={{ backgroundColor: aiThinking ? AI_COLOR : PLAYER_COLOR }} />
          ...
          <span className="text-[10px] font-bold" style={{ color: gameState === 'player_wins' ? PLAYER_COLOR : gameState === 'ai_wins' ? AI_COLOR : '#888' }}>
      ...
      <div className="flex flex-1 items-center justify-center px-2">
        <div className="relative rounded-xl p-1.5" style={{ backgroundColor: BOARD_BG }}>
          ...
          {hoverCol !== null && gameState === 'playing' && !aiThinking && board[0][hoverCol] === EMPTY && (
            <div
              ...
              style={{ width: cellSize, height: cellSize, backgroundColor: PLAYER_COLOR, left: hoverCol * (cellSize + gap) + 6, top: -cellSize - 4 }}
            />
          )}

          <div
            className="relative grid"
            style={{ gridTemplateColumns: `repeat(${COLS}, ${cellSize}px)`, gridTemplateRows: `repeat(${ROWS}, ${cellSize}px)`, gap: `${gap}px` }}
          >
            {board.map((row, r) =>
              row.map((cell, c) => (
                <div key={`${r}-${c}`} className="relative rounded-full" style={{ backgroundColor: CELL_BG, width: cellSize, height: cellSize }}>
                  <AnimatePresence>
                    {cell !== EMPTY && (
                      <motion.div
                        key={`piece-${r}-${c}`}
                        ...
                        style={{
                          ...
                          boxShadow: isWinCell(r, c) ? `0 0 8px 2px ${cell === PLAYER ? PLAYER_COLOR : AI_COLOR}` : 'none',
                        }}
                        initial={{ y: -(r + 1) * (cellSize + gap), opacity: 0.7 }}
                        animate={{ y: 0, opacity: 1, scale: isWinCell(r, c) ? [1, 1.15, 1] : 1 }}
                        transition={{
                          y: { type: 'spring', stiffness: 300, damping: 20 },
                          scale: isWinCell(r, c) ? { repeat: Infinity, duration: 1.2, ease: 'easeInOut' } : undefined,
                        }}
                      />
                    )}
                  </AnimatePresence>
      ...
      <div className="flex flex-col items-center gap-1.5 px-4 pb-3 pt-2">
        {gameState !== 'playing' && (
          <motion.button
            ...
            onClick={resetGame}
            className="rounded-full bg-peach/20 px-4 py-1.5 text-[10px] font-semibold text-peach-dim transition-colors hover:bg-peach/30"
          >
            Play Again
          </motion.button>
        )}
        <span className="text-[8px] text-black/30">Easter egg 🎮</span>
      </div>
    </div>
  );
}
