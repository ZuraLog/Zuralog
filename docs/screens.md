...
| **Conversation Drawer** | Drawer overlay | List of all past conversations sorted by most recent. Each row shows an AI-generated title, timestamp, and preview snippet. Tapping opens that thread. **Search field** at the top with `AnimatedSize` — filters conversations by title and preview text (client-side, case-insensitive). Empty-results state when no matches. Conversations can be deleted or archived from here. |
