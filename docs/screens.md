...
| **Log Grid Sheet** | Modal bottom sheet | 10-tile 4-column grid for selecting log type: Wellness, Water, Sleep, Weight, Steps, Run, Meal, Supplements, Symptom, Workout. Tapping a tile animates to its inline log panel (Wellness, Water, Weight, Steps) or opens a full-screen log screen (Sleep, Run, Meal, Supplements, Symptom — Part 3). Workout shows "coming soon" snackbar. Inline panels include submit button that logs data and invalidates `todayLogSummaryProvider`. |
...
| `ZWaterLogPanel` | `log_panels/z_water_log_panel.dart` | Inline water log: vessel picker, custom input, today's total | Part 2 |
| `ZWellnessLogPanel` | `log_panels/z_wellness_log_panel.dart` | Inline mood/energy/stress sliders + notes | Part 2 |
| `ZWeightLogPanel` | `log_panels/z_weight_log_panel.dart` | Inline weight log with kg/lbs toggle | Part 2 |
| `ZStepsLogPanel` | `log_panels/z_steps_log_panel.dart` | Inline step count entry | Part 2 |
