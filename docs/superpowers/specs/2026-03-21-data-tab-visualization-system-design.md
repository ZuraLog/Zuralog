...
**Config fields:** `points: List<ChartPoint>`, `targetLine: double?` (dashed goal line at this y-value), `fillOpacity: double` (default 0.15), `delta: double?` (percentage change for delta badge, e.g. −0.03 = ↓ 3%), `positiveIsUp: bool` (controls delta badge color semantics — `false` for weight loss where decrease is good).
...
// Note: paired metrics (e.g. blood pressure) use DualValueConfig with points1/points2,
// NOT LineChartConfig with a secondaryPoints field. LineChartConfig is always single-line.
...
**Apple Health:** `HKQuantityType(.bloodGlucose)` requires a medical device entitlement (`com.apple.developer.healthkit.background-delivery` for CGMs) that is not yet approved. The bridge method `getBloodGlucose()` is added as a stub that always returns `null`.

**Android:** `BloodGlucoseRecord` in Health Connect is available without special entitlement. Add to `REQUIRED_PERMISSIONS` and implement `readBloodGlucose(date)`. On Android, real data may be available even while iOS is in ghost state.

**UI:** `TileId.bloodGlucose` maps to `LineChartConfig` (trend) at 1×2 and `GaugeConfig` (low/normal/high zones) at 1×1. The tile always shows `noSource` ghost state until entitlement is granted and data flows.
...
Adding 11 new tile IDs to the enum (existing 20 remain unchanged):
...
enum TileId {
  ...
  walkingSpeed,      // Activity (Apple Health only; walkingSpeed HKQuantityType)
  runningPace,       // Activity (derived from workout sessions)
...
The `cycle`, `mobility`, `water`, `environment` tiles are already in the enum. `cycle`, `mobility`, and `water` need their data pipelines wired (Section 9). `environment` is an external API; see Section 12 (Out of Scope).
...
### Old → New Subtype Mapping

| Old `TileVisualizationData` subtype | New `TileVisualizationConfig` subtype | Field changes |
|---|---|---|
| `BarChartData` | `BarChartConfig` | `bars` → `bars`, `goal` → `goalValue`, add `showAvgLine: bool` |
| `RingData` | `RingConfig` | `value`, `maxValue`, `unit` unchanged; remove `showWeeklyBars: bool`; add `weeklyBars: List<BarPoint>?` |
| `LineChartData` | `LineChartConfig` | `points` unchanged; rename `positiveDirection` → `positiveIsUp`; add `referenceLine`, `rangeMin`, `rangeMax` |
| `AreaChartData` | `AreaChartConfig` | `points` unchanged; rename `target` → `targetLine`; add `fillOpacity`, `delta`, `positiveIsUp` |
| `GaugeData` | `GaugeConfig` | `value`, `minValue`, `maxValue` unchanged; `zones` format unchanged |
| `SegmentedData` | `SegmentedBarConfig` | `segments` unchanged; add `totalLabel` |
| `FillGaugeData` | `FillGaugeConfig` | All fields unchanged |
| `DotRowData` | `DotRowConfig` | `points` unchanged; `invertedScale` unchanged |
| `CalendarData` | `CalendarGridConfig` | `days` unchanged; rename `count` → `totalDays` |
| `HeatmapData` | `HeatmapConfig` | All fields unchanged |
| `StatData` | `StatCardConfig` | `value`, `unit` unchanged; rename `color` → `statusColor`, `label` → `statusLabel`; add `secondaryValue`, `trendNote` |
| `DualData` | `DualValueConfig` | All fields unchanged |
| `StackedBarData` | _(removed)_ | Was a temporary alias for `SegmentedBarConfig` — consolidate into `SegmentedBarConfig` |
| `WorkoutListData` | _(removed)_ | Workouts tile now uses `StatCardConfig` with a large count value; workout list detail moves to expanded view |

### Size-Dependent Config Type Switching

For tiles where the best visualization changes with size (not just layout), `_buildTileViz()` returns a different `TileVisualizationConfig` subtype per size. The pattern:

```dart
case TileId.steps:
  return switch (size) {
    TileSize.square => RingConfig(value: today, maxValue: goal, unit: 'steps'),
    TileSize.wide   => BarChartConfig(bars: last7Days, goalValue: goal),
    TileSize.tall   => BarChartConfig(bars: last7Days, goalValue: goal, showAvgLine: true),
  };
```

Tiles that use this pattern: `steps` (Ring at 1×1, BarChart at 2×1/1×2), `sleepDuration` (Ring at 1×1, BarChart at 1×2), `hrv` (Gauge at 1×1, LineChart at 2×1/1×2), `calories` (Ring at 1×1, SegmentedBar at 2×1/1×2), `bloodGlucose` (Gauge at 1×1, LineChart at 1×2). All other tiles return the same config subtype at every size — only internal layout differs.
