...
**Implementation change:** `metric_tile.dart` line 177 — replace `tileId.category.displayName` with `tileId.displayName`.
...
**Size behavior:**
- **1×1:** Not supported (too small to be meaningful).
...
| Metric | TileId | Default Size | Viz Type | Notes |
|---|---|---|---|---|
| Cycle | `cycle` | square (1×1) | `CalendarGrid` | Requires menstrual data types added to bridge |
...
class RingConfig extends TileVisualizationConfig {
  ...
  final String unit;
  final List<BarPoint>? weeklyBars; // for 1×2 size
  const RingConfig({...});
}
...
**Fix:** Add `blood_pressure: {'systolic': bp['systolic'], 'diastolic': bp['diastolic']}` to the `daily_metrics` map in `HealthSyncService.syncToCloud()`. Add `case "blood_pressure"` to the iOS `notifyOfChange()` switch in `HealthKitBridge.swift`.
...
### 9.8 Mindful Minutes (Not requested)
...
Adding 8 new tile IDs to the enum (existing 20 remain unchanged):
...
enum TileId {
  ...
  respiratoryRate,   // Heart (or Vitals)
...
| `tile_models.dart` | Add 8 new `TileId` values; update `displayName`, `category`, `defaultSize`, `allowedSizes` |
